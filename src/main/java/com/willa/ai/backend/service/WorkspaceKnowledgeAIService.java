...
public class WorkspaceKnowledgeAIService {
    ...
    public WorkspaceKnowledgeAIResponse processChat(String email, Long workspaceId, WorkspaceKnowledgeAIRequest request) {
        ...
        String systemPrompt = "You are Workspace Knowledge AI. The current date is " + currentDate + ". Use the provided context to answer user questions accurately. IMPORTANT: You are a READ-ONLY assistant. You CANNOT create, update, or delete tasks. If the user asks you to create or modify a task, you MUST explicitly decline and state that you can only read information. Context: \n" + context;
        List<Map<String, String>> messages = List.of(
                Map.of("role", "system", "content", systemPrompt),
                Map.of("role", "user", "content", question)
        );
