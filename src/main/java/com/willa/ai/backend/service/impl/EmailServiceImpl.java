...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendPaymentThankYouEmail(
        ...
        String openUrl = appUrl != null && !appUrl.isBlank() ? appUrl.trim() : "https://willaai.tech";
        String html = "<!DOCTYPE html><html><body style='font-family:Arial,sans-serif;color:#333;'>"
                + "<div style='max-width:560px;margin:0 auto;padding:24px;'>"
                + "<h2>Cảm ơn đã thanh toán</h2>"
                + "<p>Xin chào <strong>" + escapeHtml(name) + "</strong>,</p>"
                + "<p>Cảm ơn bạn đã thanh toán thành công trên <strong>WillaAI</strong>.</p>"
                + "<p>Chi tiết đơn:</p>"
                + "<ul>"
                ...
                + "</ul>"
                + "<p>Gói của bạn đã được kích hoạt. Chúc bạn trải nghiệm tuyệt vời!</p>"
                + "<p><a href='" + escapeHtml(openUrl) + "' style='display:inline-block;background:#8b3dff;color:#fff;"
                + "padding:12px 20px;text-decoration:none;border-radius:6px;font-weight:600;'>Mở WillA</a></p>"
                + "</div></body></html>";
        sendHtmlEmail(to, subject, html);
    }
